...
As a rule: Instruments that measure the inputs or drive the outputs directly display the actual voltage present on the connectors and expose `range` (e.g Oscilloscope, Spectrum Analyzer). Instruments that connect inputs to outputs use `attenuation` as it makes it much easier to calcuate the overall gain (e.g. PID Controller, Lock-In Amplifier, Filter Boxes). Again taking the PID Controller as an example: If the input attenuation is `20dB` and the output hardware gain is `14dB`, then your controller needs a Proportional gain of `6dB` to maintain a `0dB` proportional gain from input to output.
