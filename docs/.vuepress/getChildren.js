...
const getDirContent = function (path, isRoot = false) {
    files = glob.sync(path + '**/*.md').map((path) => {
    ...
    var sortedfiles = _.sortBy(files, ['path']).map((file) => [
        file.path,
        file.f_name,
    ])
...
const getChildren = function (title, lookup_path) {
    complete_path = 'docs/api/' + lookup_path

    sub_directories = fs
        .readdirSync(complete_path, { withFileTypes: true })
