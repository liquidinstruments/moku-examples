...
def add_glitch_gaussian(
    ...
    if random_variables:
        std_dev = 0.1
        glitch_factor = max(
            0.1, min(0.9, random.gauss(mean, std_dev))
        )  # Clamp to avoid flat signal
...
def generate_noisy_sine_wave(amplitude_value, phase_shift_value, snr_percentage_value):
    ...
    v_out, t_out, amplitude = generate_clean_sine_100(
        amplitude=amplitude_value, phase_shift=phase_shift_value, random_parameters=False
    )
...
def generate_noisy_sine_wave_with_glitch(
    amplitude_value, phase_shift_value, index_position, csv_file, glitch_factor_value=2, mean=0.125
):
    phase_shift_value += random.uniform(0, phase_shift_value * 0.05)
    ...
    signal, glitch_width_p, glitch_factor = add_glitch_gaussian(
        v_out,
        t_out,
        amplitude,
        glitch_factor=glitch_factor_value,
        random_variables=False,
        index_pos=index_position,
        mean=mean,
        save_to_file=True,
        csv_file=csv_file,
    )
...
def create_dataset_glitch(
    num_samples=5000,
    amplitude_value=1,
    anomaly_percentage=0.01,
    glitch_factor_value=0.8,
    folder_path="training_dataset_ad/",
):
    ...
    for i in range(num_samples):
        ...
        if i in anomaly_indexes:  # This is a frame with an anomaly
            signal = generate_noisy_sine_wave_with_glitch(
                amplitude_value,
                phase_shift_value,
                index_position=i,
                csv_file=anomaly_file_value,
                glitch_factor_value=glitch_factor_value,
                mean=glitch_factor_value,
            )
            signal_type = 'anomaly'
        else:
            signal = generate_noisy_sine_wave(
                amplitude_value, phase_shift_value, snr_percentage_value
            )
            signal_type = "normal"
...
def plot_reconstruction(original, reconstructed, frame_n):
    ...
    plt.plot(
        reconstructed_signal[frame_n], label='Reconstructed Signal', color='orange', linestyle='--'
    )
...
class SineWaveDataset:
    def __init__(
        self,
        num_samples=5000,
        training_dataset_with_anomaly=False,
        file_path="sine_wave_dataset.csv",
    ):
        self.data = []
