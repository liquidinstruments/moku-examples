...
try:
    ...
    mg.generate_waveform(channel=1, type='Sine', amplitude=0.1, frequency=2e3)
    mg.generate_waveform(
        channel=2, type='Pulse', pulse_width=0.005, edge_time=16e-9, amplitude=1, frequency=10
    )
    ...
    for cnt in range(1, 21, 1):
        ...
        dl.start_streaming(0.2, trigger_source='InputB', trigger_level=0.01)
        dl.stream_to_file('dataStream' + str(cnt) + '.csv')
        time.sleep(5)
        dl.stop_streaming()
        mg.generate_waveform(channel=1, type='Sine', amplitude=cnt * 0.1, frequency=2e3)
