...
for cnt in range(20, 0, -1):
    ...
    pulse_duration = 0.005  # Duration of the pulse (5 milleseconds)
    ...
    plt.subplot(3, 2, 1)
    plt.plot(
        time[int(time.size * 0.45) : int(time.size * 0.55)],
        reference[int(time.size * 0.45) : int(time.size * 0.55)],
        linewidth=0.2,
    )
    ...
    plt.plot(
        time[int(time.size * 0.45) : int(time.size * 0.55)],
        noisey[int(time.size * 0.45) : int(time.size * 0.55)],
        linewidth=0.1,
    )
    ...
    plt.plot(
        time[int(time.size * 0.45) : int(time.size * 0.55)],
        autoencoder[int(time.size * 0.45) : int(time.size * 0.55)],
        linewidth=0.2,
    )
    ...
    maxCorrRef2 = np.argmax(correlationRef[int((time.size) / 3) + 1 : time.size]) + int(
        (time.size) / 3
    )
    ...
    plt.legend(loc=1)
    plt.plot(time, correlationRef[0 : time.size], linewidth=0.1)
    ...
    plt.axhline(y=maxy / 2, color='red', linestyle='--', label='Detection Threshold')
    plt.axvline(
        x=time[maxCorrRef1], color='green', linestyle='--', label='Expected Detection', linewidth=1
    )
    plt.axvline(x=time[maxCorrRef2], color='green', linestyle='--', linewidth=1)
    plt.legend(loc=1)
    plt.plot(time, correlationNoisey[0 : time.size], linewidth=0.1)
    ...
    plt.axvline(x=time[maxCorrRef2], color='green', linestyle='--', linewidth=1)
    plt.legend(loc=1)
    plt.plot(time, correlationAuto[0 : time.size], linewidth=0.1)
    plt.title('Matched Filter on Autoencoder De-Noised Received Pulse')
    plt.xlabel('Time (s)')
    plt.ylabel('Amplitude')

    cols = [
        '200ms Excerpt of Pulse Data at ' + str(round(cnt * 0.01, 2)) + 'Vpp',
        'Matched Filter Correlation on ' + str(round(cnt * 0.01, 2)) + 'Vpp Pulse',
    ]
