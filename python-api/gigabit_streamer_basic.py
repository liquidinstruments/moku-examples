...
try:
    ...
    gs1.set_acquisition(mode='Normal', sample_rate=5e3)
    ...
    gs1.set_local_network(ip_address="10.10.1.2", port=5000)
    # Get the MAC address of Gigabit Streamer 2 (connected to SFP2)
    gs2_mac = gs2.set_local_network(ip_address="10.10.1.1", port=5000)["mac_address"]
    gs1.set_remote_network(ip_address="10.10.1.1", port=5000, mac_address=gs2_mac)
    gs1.set_outgoing_packets(mtu="1500bytes")

    # Set up the transmitting Gigabit Streamer instrument
    #########################################################
    # Configure the acquisition settings
    gs2.set_acquisition(mode='Normal', sample_rate=5e3)
    gs2.set_interpolation(mode='Linear')
    gs2.enable_input(channel=1, enable=True)
    gs2.enable_input(channel=1, enable=False)

    # Configure the local and remote network settings
    gs2.set_local_network(ip_address="10.10.1.1", port=5000)
    # Get the MAC address of Gigabit Streamer 1 (connected to SFP1)
    gs1_mac = gs1.set_local_network(ip_address="10.10.1.2", port=5000)["mac_address"]
    ...
    gs1.set_output(channel=2, enable=True, gain=0.0, offset=0.0)
    gs2.set_output(channel=1, enable=True, gain=0.0, offset=0.0)
    gs2.set_output(channel=2, enable=True, gain=0.0, offset=0.0)
