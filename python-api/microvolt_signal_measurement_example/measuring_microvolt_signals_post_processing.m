...
for k = 1:length(files)
    ...
    [error(k), time_chunk, voltage_chunk] = process_PM_data(folder, filename);
    
    % Store time values for initial message in measurements for plotting
    if k == 1
        all_times = time_chunk;
    end
    
    % Save first message voltages for plotting
    all_voltages(k,:) = voltage_chunk;
end
...
function [overall_avg_error, first_time, first_voltage] = process_PM_data(folder,filename)
    ...
    chunk_duration = 33.33; % Period based on AWG waveform frequency
    ...
    all_segment_mids = [];  % will be a matrix: rows = chunks, cols = segments
    count = 0;
    
    % Loop through each 33.33-second chunk
    while start_time + chunk_duration <= time_full(end) && count < 10
        count = count + 1;
        % Define time window for this chunk
        end_time = start_time + chunk_duration;
        indices = (time_full >= start_time) & (time_full <= end_time);
    
        % Extract time and voltage for this chunk
        time = time_full(indices);
        voltage = voltage_full(indices)-offset;
    
        % Segment & band logic
        range = 0.5; 
        bottom_third = -0.25 + (range*0.333);
        top_third = 0.25 - (range*0.333);
        ...
        num_segments = 14; % Number of Morse characters
        top = 0.25; 
        mid = 0; 
        bottom = -0.25;

        % Make list of encoded Morse levels (top = dash, bottom = space,
        % mid = dot)
        ...
        fprintf('Chunk starting at %.2f s — Morse Code Message: %s\n', start_time, message);
    ...
    % Compute absolute error per chunk and per segment
    abs_error_matrix = (abs(all_segment_mids - segment_refs)/segment_refs)*100;  % size: [chunks x segments]
