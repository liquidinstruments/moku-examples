...
m = MokuMultiInstrument('192.168.###.###', 3, true);
...
try
    ...
    gs1.set_acquisition('Normal', 5e3);
    gs1.set_interpolation('Linear');
    gs1.enable_input(1, 'enable', true);
    gs1.enable_input(2, 'enable', true);

    % Configure the local and remote network settings for gs1
    gs1.set_local_network('10.10.1.2', 5000);
    % Get the MAC address of Gigabit Streamer 2 (connected to SFP2)
    result = gs2.set_local_network('ip_address', '10.10.1.1', 'port', 5000);
    gs2_mac = result.mac_address;
    gs1.set_remote_network('10.10.1.1', 5000, gs2_mac);
    gs1.set_outgoing_packets('1500bytes');

    %% Set up the transmitting Gigabit Streamer instrument
    % Configure the acquisition settings
    gs2.set_acquisition('Normal', 5e3);
    gs2.set_interpolation('Linear');
    gs2.enable_input(1, 'enable', true);
    % Example toggling input off then on (mirrors python example intent)
    gs2.enable_input(1, 'enable', false);

    % Configure the local and remote network settings for gs2
    gs2.set_local_network('10.10.1.1', 5000);
    % Get the MAC address of Gigabit Streamer 1 (connected to SFP1)
    result = gs1.set_local_network('ip_address', '10.10.1.2', 'port', 5000);
    gs1_mac = result.mac_address;
    gs2.set_remote_network('10.10.1.2', 5000, gs1_mac);
    gs2.set_outgoing_packets('1500bytes');

    % Enable outputs and start sending data
    gs1.set_output(1, true,'gain',0.0,'offset',0.0);
    gs1.set_output(2, true,'gain',0.0,'offset',0.0);
    gs2.set_output(1, true,'gain',0.0,'offset',0.0);
    gs2.set_output(2, true,'gain',0.0,'offset',0.0);
